...
CREATE OR REPLACE VIEW ignition_demo.renewables_co_ot.silver_signals_1m AS
WITH base AS (
  SELECT
    ...
    tag_path,
    event_time
  FROM ignition_demo.renewables_co_ot.silver_events_normalized
...
CREATE OR REPLACE VIEW ignition_demo.renewables_co_ot.silver_signals_latest AS
SELECT
  asset_id,
  signal_name,
  unit,
...
CREATE OR REPLACE VIEW ignition_demo.renewables_co_ot.silver_grid_events AS
WITH x AS (
  SELECT
    ts_1m,
...
CREATE OR REPLACE VIEW ignition_demo.renewables_co_ot.silver_maintenance_events AS
WITH x AS (
  SELECT
    ts_1m,
