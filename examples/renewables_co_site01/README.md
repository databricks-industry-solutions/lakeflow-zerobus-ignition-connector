### Renewables Co “Site01” simulation (Ignition) — end-to-end OT demo
...
- **`renewables_co_site01_tags.json`**: Import into provider **`tilt`**.
- **`renewables_co_grid_site01_tags.json`**: Import into provider **`grid`**.
- **`renewables_co_cmms_site01_tags.json`**: Import into provider **`cmms`**.
- **`renewables_co_forecast_site01_tags.json`**: Import into provider **`forecast`**.

Gateway Timer Scripts (create 4 timers at 1s or 2s):

- **`timer_script_renewables_co_site01_plant_telemetry.py`**
- **`timer_script_renewables_co_site01_grid_market.py`**
- **`timer_script_renewables_co_site01_maintenance_events.py`**
- **`timer_script_renewables_co_site01_weather_forecast.py`**
...
## Demo tag model (Renewables Co multi-asset)
...
- For each provider, select it and import its JSON:
  - Provider `tilt` → import `renewables_co_site01_tags.json`
  - Provider `grid` → import `renewables_co_grid_site01_tags.json`
  - Provider `cmms` → import `renewables_co_cmms_site01_tags.json`
  - Provider `forecast` → import `renewables_co_forecast_site01_tags.json`
