### Petrol Co — Oil & Gas demo tag model (Ignition) — example
...
- Provider `pluspetrol` → `petrol_co_site01_tags.json`
- Provider `pluspetrol_safety` → `petrol_co_site01_safety_tags.json`
- Provider `pluspetrol_integrity` → `petrol_co_site01_integrity_tags.json`

Gateway Timer Scripts:

- `timer_script_petrol_co_site01.py` (SCADA/process, 1000ms)
- `timer_script_petrol_co_site01_safety.py` (safety baseline, 2000ms)
- `timer_script_petrol_co_site01_integrity.py` (integrity baseline, 10000ms)
- `timer_script_petrol_co_site01_upsets.py` (optional upsets, 5000–10000ms)
...
- `timer_script_petrol_co_site01_write_only.py` (process, 1000ms)
- `timer_script_petrol_co_site01_safety_write_only.py` (safety, 2000ms)
- `timer_script_petrol_co_site01_integrity_write_only.py` (integrity, 10000ms)
...
- Right-click → **Import**
- Choose **JSON**
- Import `examples/petrol_co_site01/petrol_co_site01_tags.json`

Repeat for:

- Provider **`pluspetrol_safety`** → import `petrol_co_site01_safety_tags.json`
- Provider **`pluspetrol_integrity`** → import `petrol_co_site01_integrity_tags.json`
...
- Set:
  ...
  - **Delay (ms)**: 1000
  - **Enabled**: true
- Create a timer (1000ms) and paste `timer_script_petrol_co_site01.py` (SCADA/process).

Then create:

- A timer (2000ms) for `timer_script_petrol_co_site01_safety.py`
- A timer (10000ms) for `timer_script_petrol_co_site01_integrity.py`
...
- Create a **second** timer event with:
  ...
  - **Enabled**: true
- Paste `examples/petrol_co_site01/timer_script_petrol_co_site01_upsets.py` (this writes to both `pluspetrol` and `pluspetrol_safety`)
