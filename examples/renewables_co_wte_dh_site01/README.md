## Renewables Co (Site01) — Waste-to-Energy + District Heating + Emissions demo

This demo models the kind of OT data a municipal utility would have in SCADA:
...
- Provider `[kredsloeb_process]` → `renewables_co_process_site01_tags.json`
- Provider `[kredsloeb_dh]` → `renewables_co_dh_site01_tags.json`
- Provider `[kredsloeb_env]` → `renewables_co_env_site01_tags.json`

### 3) Run a single simulator timer

Create **one Gateway Timer Script** (e.g. **1000 ms**) and paste:

- `timer_script_renewables_co_site01_orchestrator.py`
