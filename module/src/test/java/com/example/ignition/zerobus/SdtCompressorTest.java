...
public class SdtCompressorTest {
    ...
    void firstSampleEmits() {
        ...
        SdtCompressor.Outcome o = s.offer(te("t", 1.0, 1000L), 0.1, 0);
        assertNotNull(o);
    ...
    void linearWithinDeviationDoesNotEmit() {
        ...
        assertNotNull(s.offer(te(path, 0.0, 0L), dev, 0).emit);
        assertNull(s.offer(te(path, 0.2, 1000L), dev, 0).emit);
        assertNull(s.offer(te(path, 0.4, 2000L), dev, 0).emit);
        assertNull(s.offer(te(path, 0.6, 3000L), dev, 0).emit);
    }
    ...
    void doorClosureEmitsPreviousPoint() {
        ...
        assertNull(s.offer(te(path, 0.0, 1000L), dev, 0).emit);

        // Large jump should close the door and emit the previous point (t=1000)
        SdtCompressor.Outcome o = s.offer(te(path, 10.0, 2000L), dev, 0);
        assertNotNull(o.emit);
    ...
    void maxIntervalForcesEmit() {
        ...
        assertNotNull(s.offer(te(path, 5.0, 0L), dev, 1000).emit);
        // No change within interval -> no emit
        assertNull(s.offer(te(path, 5.0, 500L), dev, 1000).emit);

        // At interval boundary -> forced emit
        SdtCompressor.Outcome forced = s.offer(te(path, 5.0, 1000L), dev, 1000);
    ...
    void outOfOrderResetsAndEmitsCurrent() {
        ...
        assertNotNull(s.offer(te(path, 1.0, 1000L), 0.1, 0).emit);

        SdtCompressor.Outcome o = s.offer(te(path, 2.0, 900L), 0.1, 0);
        assertNotNull(o.emit);
